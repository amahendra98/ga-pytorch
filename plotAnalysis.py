...
def InferenceAccuracyExamplePlot(model_name, save_name, title, sample_num=10, fig_size=(15, 5), random_seed=1,
    ...
    targets = target_region[0] + (target_region[1] - target_region[0]) * np.random.uniform(low=0, high=1,
                                                                                           size=10)  # Cap the random numbers within 0-299
...
def PlotPossibleGeoSpace(figname, Xpred_dir, compare_original=False, calculate_diversity=None):
    ...
    if (calculate_diversity != None):
        plt.text(-4, 3.5, 'Div_Xpred = {}, Div_Xtruth = {}, under criteria {}'.format(diversity_Xpred, diversity_Xtruth,
                                                                                      calculate_diversity), zorder=1)
...
def MeanAvgnMinMSEvsTry(data_dir):
    ...
    for ind, yp in enumerate(Ypred_list):
        if np.shape(yp) != np.shape(Yt):
            print(
                "Your Ypred file shape does not match your ytruth, however, we are trying to reshape your ypred file into the Ytruth file shape")
...
def MeanAvgnMinMSEvsTry_all(data_dir):  # Depth=2 now based on current directory structure
    ...
    for dirs in os.listdir(data_dir):
        ...
        for subdirs in os.listdir(os.path.join(data_dir, dirs)):
            if os.path.isfile(os.path.join(data_dir, dirs, subdirs,
                                           'mse_min_list.txt')):  # Dont do for gaussian first and if this has been done
                continue;
...
def DrawAggregateMeanAvgnMSEPlot(data_dir, data_name, save_name='aggregate_plot',
                                 gif_flag=False):  # Depth=2 now based on current directory structure
    """
    ...
    eval_time_dict_robotic_arm = {"Backprop": 2.84, "Tandem": 0.17, "VAE": 0.25, "INN": 0.40, "cINN": 0.79,
                                  "Random": 0.1}
    eval_time_dict_sine_wave = {"Backprop": 7.19, "Tandem": 0.55, "VAE": 0.80, "INN": 0.95, "cINN": 3.16, "Random": 0.1}
    eval_time_dict_meta_material = {"Backprop": 96.47, "Tandem": 0.55, "VAE": 0.26, "INN": None, "cINN": 0.27,
                                    "Random": 0.1}
    eval_time_dict_ballistics = {"Backprop": None, "Tandem": 0.17, "VAE": 0.25, "INN": None, "cINN": 0.79,
                                 "Random": 0.1}
    ...
    def plotDict(dict, name, data_name=None, logy=False, time_in_s_table=None, plot_points=51, avg_dict=None,
                 resolution=10, err_dict=None):
        """
        ...
        color_dict = {"NA": "g", "Tandem": "b", "VAE": "r", "cINN": "m",
                      "INN": "k", "Random": "y", "MDN": "violet", "Tandem__with_boundary": "orange",
                      "NA__boundary_prior": "violet", "NA__no_boundary_prior": "m", "INN_new": "violet",
        ...
        for key in sorted(dict.keys()):
            ...
            # print(dict[key])
            if err_dict is None:
                plt.plot(x_axis[:plot_points:resolution], dict[key][:plot_points:resolution], c=color_dict[key],
                         label=key)
            else:
                print(np.shape(err_dict[key]))
                plt.errorbar(x_axis[:plot_points:resolution], dict[key][:plot_points:resolution], c=color_dict[key],
                             yerr=err_dict[key][:, :plot_points:resolution], label=key.replace('_', ' '),
                             capsize=5)  # , errorevery=resolution)#,
    ...
    plotDict(min_dict, '_minlog_quan2575.png', logy=True, avg_dict=avg_dict, err_dict=quan2575_dict,
             data_name=data_name)
...
def DrawEvaluationTime(data_dir, data_name, save_name='evaluation_time', logy=False, limit=1000):
    ...
    for key in sorted(eval_time_dict.keys()):
        ...
        plt.plot(np.arange(len(eval_time_dict[key])), eval_time_dict[key],
                 label=key + 'average_time={0:.2f}s'.format(average_time))
    plt.legend()
